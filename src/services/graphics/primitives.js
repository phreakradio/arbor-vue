...
class Oval extends Primitives{
    constructor(ctx,id,x,y,w,h,style){
        super(ctx);
        this.id = id;
        this.update(x,y,w,h,style);
    }

    update(x,y,w,h,style){
        this.x = x;
        this.y = y;
        this.w = w;
        this.h = h;
...
class Rectangle extends Primitives{
    constructor(ctx,id,x,y,w,h,r,style){
        super(ctx);
        ...
        this.id = id;
        this.update(x,y,w,h,r,style);
    }

    update(x,y,w,h,r,style){
        if (has(r, ['stroke', 'fill', 'width'])){
            style = r;
            r = 0;
        }
...
class Path extends Primitives{
    constructor(ctx, id, x1, y1, x2, y2, style){
        super(ctx);
        this.id = id;
        // calling patterns:
        // ƒ( x1, y1, x2, y2, <style> )
...
export {Oval, Rectangle, Path, Primitives};