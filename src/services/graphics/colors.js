...
const _isRGB = (color) => {
    ...
    var components = keys(color).sort().join("");
    if (components == 'abgr') return true;
};

/*eslint no-useless-escape: "error"*/
const nano = function(tpl, data) {
...
export const Colors = {
    ...
    decode : function(clr){
        ...
        if (typeof clr=='string'){
            ...
            var nameMatch = Colors.CSS[clr.toLowerCase()];
            if (nameMatch!==undefined){
                clr = nameMatch;
            }
            var hexMatch = clr.match(iscolor_re);
            if (hexMatch){
                let vals = clr.match(hexrgb_re);
    ...
    validate : function(str){
        ...
        if (Colors.CSS[str.toLowerCase()] !== undefined) return true;
    ...
    blend : function(rgbOrHex, alpha){
        ...
        var rgb = Colors.decode(rgbOrHex);
        if (!rgb) return null;

        if (alpha==1) return rgbOrHex;
        rgb = rgbOrHex;
        if (typeof rgbOrHex=='string') rgb = Colors.decode(rgbOrHex);

        var blended = cloneDeep(rgb);
        blended.a *= alpha;
