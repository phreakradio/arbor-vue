...
import Point from "./physics/atoms.js";
	...
	init(system){
	  ...
	  redraw:function(){
		...
		particleSystem.eachEdge(function(edge, pt1, pt2){
		  ...
		  gfx.line(pt1, pt2, {stroke:colour.gray, width:1, alpha:edge.target.data.alpha})
		})
		...
		particleSystem.eachNode(function(node, pt){
		  ...
		  if (node.data.shape=='dot'){
			...
			if (imageob){
			  // Images are cached 
			  ctx.drawImage(imageob, pt.x-(imageW/2), pt.y-radius/4, imageW, imageH)
			}
		  }else {
	  ...
	  initMouseHandling:function(){
		...
		var handler = {
		  moved:function(e){
			...
			//}else if ($.inArray(nearest.node.name, ['python', 'java','terminal','web','cplus']) >=0 ){
			}else if ($.inArray(nearest.node.name, ['python', 'java','android','web','cplus']) >=0 ){
